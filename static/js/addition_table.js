                        ...
                        const addition_id = row.cells[0].data;
                         return html(
                               `<span style="display: flex; justify-content: space-between">` +
                                    `<button type="button" class="view-btn additions" data-id=${addition_id}>View</button>` +
                                    `<button data-csrf-token=${csrf_token} type="button" class="delete-btn additions" data-id=${addition_id}>Delete</button>`+
                               `</span>`
                           );
                    }
        }

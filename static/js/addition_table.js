    ...
    columns: [
        ...
        },
        {
            ...
            formatter: (cell, row) => {
                        ...
                        let action_html = `<button type="button" class="view-btn additions" data-id=${addition_id}>Bax</button>`
                        if(role === "admin") {
                            action_html += `<button data-csrf-token=${csrf_token} type="button" class="delete-btn additions" data-id=${addition_id}>Sil</button>`
                        }
...
if (gridHeader) {
  ...
  titleElement.textContent = "Əlavələr";
